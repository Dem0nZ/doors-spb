      ...
      <Container>
        ...
        <Feedback>
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <input className='col-span-2 border-b-2 outline-none'
                   placeholder='Ваш Email*'{...register('email', { required: true })} />
            <input className='col-span-2 border-b-2 outline-none'
                   placeholder='Ваш телефон*' {...register('phone', { required: true })} />
            <textarea className='col-span-2 border-b-2 outline-none resize-none' rows={4}
                      placeholder='Ваше сообщение' {...register('message', { required: true })} />
            {(Object.keys(errors).length > 0) &&
              <ErrorMessage>{errors?.name && 'имя'} {errors?.email && 'почта'} {errors?.phone && 'телефон'} {errors?.message && 'сообщение'} обязательны
                для отправки формы</ErrorMessage>}
